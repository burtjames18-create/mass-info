...
export default function StockPage({
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10">
      ...
      <div className="flex items-start justify-between mb-10 animate-fade-in-up">
        <div>
          <div className="flex items-center gap-4 mb-1">
            <h1 className="text-2xl font-light tracking-[0.1em] text-white">{upperTicker}</h1>
            <button
              onClick={addToWatchlist}
              className="text-xs tracking-[0.2em] uppercase text-white/25 border border-white/12 hover:border-white/40 hover:text-white px-3 py-1 transition-all"
            >
              + Watch
            </button>
          </div>
        ...
        <div className="text-right animate-count-up">
          <div
            className="text-3xl font-light tracking-tight transition-all duration-100"
            style={{
      ...
      {/* Controls */}
      <div className="flex items-center gap-2 mb-4 animate-fade-in stagger-1 flex-wrap">
        {periods.map((p) => {
          const isLive = p === "1d";
          ...
          return (
            <button
              key={p}
              onClick={() => setPeriod(p)}
              className={`flex items-center gap-1.5 px-3 py-1 text-xs tracking-widest uppercase border transition-all ${
                isActive
        ...
        <div className="ml-2 flex border border-white/10">
          <button
          ...
          <button
            ...
            className={`px-3 py-1 text-xs tracking-wider uppercase border-l border-white/10 transition-all ${
        ...
        {!isIntraday && (
          <div className="ml-auto flex items-center gap-2">
            <span className="text-xs text-white/20 tracking-widest uppercase">Forecast</span>
            {[14, 30, 60, 90].map((d) => (
              <button
                key={d}
      ...
      <div className="animate-fade-in-up stagger-2 mb-4">
        {historyLoading ? (
          <div className="w-full h-[500px] shimmer" />
          ...
          <div className="w-full h-[500px] border border-white/8 flex items-center justify-center">
